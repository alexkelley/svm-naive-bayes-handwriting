...
def plot_linear_pc1_v_pc2(df, X_columns, y_column):
    X_data = df[X_columns]
    ...
    ax.set_title('Average Value by Label\nLinear PC1 versus PC2',
                 fontsize=18,
                 color="b",
                 alpha=0.5)
    ax.set_xlabel('PC1',
...
def plot_kernel_pc1_v_pc2(df, X_columns, y_column):
    X_data = df[X_columns]
    pcs = fit_kernel_PCA(X_data, 2)
    pcs[y_column] = df[y_column]
