...
def evaluate_linear_pcs(X_data, n_components):
    results = 'Principal Component Variance'
    results += '\n{}\n'.format(len(results) * '=')
...
def select_feature_sample(df, X_columns):
    '''
    Select a random sub-sample of features to include in the model
    '''
    n_features = int(len(X_columns) * 0.50)
    # drop first few columns and last few columns
    X_new = X_columns[5:-5]
...
def nb_trial(samples):
    ...
    scores = {
        'accurracy_train': acc_train,
        'accurracy_test': acc_test
